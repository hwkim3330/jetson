...
def generate_launch_description():
    ...
    usb_port = LaunchConfiguration('usb_port', default='/dev/ttyTHS1')

    if ROS_DISTRO == 'humble':
        rsb_param_dir = LaunchConfiguration(
            'rsb_param_dir',
    ...
    elif LDS_MODEL == 'LDS-04':
        lidar_pkg_dir = LaunchConfiguration(
            'lidar_pkg_dir',
            default=os.path.join(get_package_share_directory('ldlidar_stl_ros2'), 'launch'))
        LDS_LAUNCH_FILE = '/ld19.launch.py'        
    else:
        lidar_pkg_dir = LaunchConfiguration(
            'lidar_pkg_dir',
            default=os.path.join(get_package_share_directory('hls_lfcd_lds_driver'), 'launch'))
    ...
    return LaunchDescription([
        ...
        # CSI Camera with nvjpegenc + MJPEG HTTP server (port 8080)
        Node(
            package='robot_web',
